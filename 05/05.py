    ...
    for v in vents:
        if v.start.x == v.end.x:
            if v.start.y < v.end.y:
                ...
                ys = range(v.end.y, v.start.y + 1)
            for y in ys:
                overlap_counts.update([(Point(v.start.x, y), overlap_counts[Point(v.start.x, y)] + 1)])
        elif v.start.y == v.end.y:
            if v.start.x < v.end.x:
                xs = range(v.start.x, v.end.x + 1)
            else:
                xs = range(v.end.x, v.start.x + 1)
            for x in xs:
                overlap_counts[Point(x, v.start.y)] += 1
